...
class CGAnimationSequenceController extends BaseNode {
  constructor() {
    super();
    this.component = null;
    this.haveRegistererdCallback = false;
  }
  ...
  registerEventListeners(sys) {
    ...
    this.haveRegistererdCallback = true;
  }

  onUpdate(sys, dt){
    if (this.inputs[4]()!=null && !this.haveRegistererdCallback) {
      this.registerEventListeners(this.sys);
    }
  }

  onCallBack(userData, info, eventType) {
    let animSeqComp = info.animSeqCom;
    if (animSeqComp === null || animSeqComp === undefined || this.component == null) {
      return;
    }
  ...
  execute(index) {
    ...
    if (this.inputs[4]()!=null) {
      if(!this.haveRegistererdCallback){
        this.registerEventListeners(this.sys);
      }
